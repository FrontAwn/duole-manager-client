<template>
	<div id="DailyReportCurrentStockSearch">
		<div class="admin-component-inner-box">
			<Card class="admin-iview-core-card">
				...
				<div class="admin-component-margin-bottom">货号查询：</div>
				<Input 
					placeholder="请输入货号..."
		...
		<div class="admin-component-inner-box" v-if="show">
			<Card class="admin-iview-core-card">
				...
				<div style="width: 100%;display: flex;">
					<div 
						v-for="(title,idx) in datas.titles"
						:class="idx===datas.titles.length-1?'title-table-border-end title-table-style':'title-table-border title-table-style'"
					>
						{{title}}
					</div>
				</div>
				<div 
					v-for="(nums,option) in datas.buildedDatas"
					style="width: 100%;display: flex;"

				>
					<div style=""
						class="content-table-border content-table-style"
					>{{option}}</div>
					<div 
						v-for="(num,idx) in nums"
						:class="idx === nums.length-1?'content-table-border-end content-table-style':'content-table-border content-table-style'"
					>{{num}}</div>
				</div>
